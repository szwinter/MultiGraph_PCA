...
for h=1:K
    ...
    while(Local_Search_Iter>localsiter)
        ...
        for i=1:r
            % currently not sure how to measure convergence
            % should we go until the max objective is below some threshold?
            %
            % or should we go until the penalty is below the threshold?
            %
            % I'll try computing the penalty method first?
            %
